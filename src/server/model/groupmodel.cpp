...
void GroupModel::addToGroup(int groupid, int userid, string role)
{
    char sql[1024] = {0};
...
vector<Group> GroupModel::queryGroups(int userid)
{
    ...
    sprintf(sql, "select a.id, a.groupname, a.groupdesc from allgroup a inner join \
        groupuser b on a.id = b.groupid where userid = %d", userid);
...
vector<int> GroupModel::queryUsersInGroup(int userid, int groupid)
{
    char sql[1024] = {0};
